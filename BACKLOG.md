...
| 3.17 | Direct-damage combat: the whole number set is a first pass | **OPEN** | Escalation removed 2026-08-30 — hits chip HP directly (`PlayerCharacter.OnHit`), `MaxHealth` 3 → 5, `CombatState.DamagePercent` gone, replaced by the read-only `DamageTaken` tally. Every number that moved is a first guess and none has been felt in-game: the pool (5) against a 0.5 slash and a 1.0 creature swing; the ~40% cut to every slash `StrikeSpeed` and `SlashMinLaunch` 180 → 100; the creature knockbacks (base melee 460 → 250, lunge 540 → 300, brute dive 650 → 360) and the projectile set (bullet 1200 → 400, rail bolt 950 → 350, energy ball 540 → 330, grenade 700 → 430, thrown clod 520 → 320, charged blast 640 → 400); `StunImpulseThreshold` 440 → 280 and `HitstunSecondsPerImpulse` 0.00135 → 0.0022, which were rescaled *together* so the same moves stun and the disadvantage windows are unchanged — check that pairing first if hitstun feels wrong. `StabAction.StrikeSpeed` (650) was deliberately left alone so the stab is the kit's one launcher; it is now ~2.2× GroundSlash3, up from ~1.3×. Regen went 0.8/s behind a 1 s delay to 0.15/s behind 3 s, and its anchor moved from crush-only to any damage — a fight is no longer out-healable, which is a real change in pacing. Also new and untested: `Hitbox.BodyDamage`, which lets the four stone-carving attacks (Zeus bolt/sweep/column, rail bolt) hurt a body less than they carve a tile; without it a beam that must deal 2.6 to clear a stone cell two-shots a player. `ArmorStrength` 300 → 190 followed the strength rescale so a Slash3 still breaks a stab's superarmor. |
...
Preserved verbatim from `todo.txt` — unresolved, and the reason the escalation model looked the
way it did. (Escalation itself is gone as of 2026-08-30 — hits chip HP directly now — but the
question below is about hitstun and disadvantage states, which survived the change unchanged.)
...
| 5.8 | Surface-relative descent limiting (sprout-style moving floors) wants surface velocity in the solve. | **PARTIAL** | 2026-08-31: standing + jumping done, contact-scoped (never a single joined frame). Rise gates measure against the support surface's velocity (`FoldBaseline`, qp `launched`, `FoldReference.Admit`, `LatticeTracker`, `PredictFold`); `FloorEnvelope` + the predictor's `TryProbeFloor` see growing sprout volumes as floor; station friction brakes toward the surface's tangential velocity. Jumps already launched source-relative. Tests: `Sim/SproutLiftJumpTests.cs` (the chain test pins the old gravity-fight sawtooth at 1.5px/frame lurch vs 0.82 now). Multi-directional carry: `TerrainCarriedState` (`Character/Movement/TerrainCarriedState.cs`, priorities 22/22) rides the per-contact aggregate push when it has a real horizontal component — gravity held, no station friction against the ride, ensure-at-least along the aggregate; pure-vertical lifts stay Standing's. Enabled by two physics-fidelity fixes in `PhysicsWorld`: moving-surface contacts advect their anchor and get a `surfaceSpeed·dt` prune allowance (they used to die at the end of every step), and `TryFindContactSurface` refreshes from the fastest-advancing shape instead of first-found (an edge-flush wall tile used to zero a wall-grown sprout contact's velocity). Diagonal streams: multi-face sprouts now emit ONE combined volume translating along the summed parent offset (`TileSproutNode.TryCombinedOffset` — true crest velocity on the top face; opposed faces fall back to per-face), and the carry uses a pushing contact's FULL surface velocity, deduped per mover — riding on top of a diagonal eruption carries diagonally instead of straight up and out. The macroscale ride (`SproutLiftJumpTests.DiagonalEruptionStream…`: a 2-wide diagonal cascade bar carrying the player ~22 tiles up-and-right, no input) required three findings, all encoded in `TerrainCarriedState`: (1) the VERTICAL half of any ride is fold-driven, never contact-driven — the fold hovers the body ~15px up, more than a tile, so a cell growing beneath a hovering body never touches it; the carried state must keep `FoldProfile.Stand`; (2) ride memory is a per-axis coyote envelope (`MovementVars.CarryVelocity/CarryHoldX/Y`) because stream cells push in alternating axis bursts; (3) the control law is the ANCHOR SERVO (`TerrainCarriedState.RideTarget`): each growing volume within ~1.5 tiles moving toward the body projects a velocity field `s(lead) = clamp(speed + Kp·(standoff − lead), 0, speed)` along its own motion; the DISTANCE-WEIGHTED FLOW AVERAGE of those fields — Σw·field / (Σw + `FlowCenterBias`) — is tracked by ONE acceleration-capped velocity servo on both axes (Y engaged only while the flow has a vertical part), over the gravity-hold feedforward, with clearance-only ambient (`FoldProfile.None`) so exactly one controller owns the trajectory. The mean of the front cells' motions IS the crest's advance rate (a cascade's alternating lateral+vertical cells average to exactly the front velocity), so the macro ride is a near-constant-velocity glide (~(44, −48) px/s steady for 85+ frames, vs a scoop-relax rhythm under the per-axis-max law); the normalization bias makes thinning flow at a stream's fringe pull the target down — a mild centering force easing the rider back in. The same query is CONTINUITY evidence (any approaching mass keeps the classification alive): `carried runs: 1` over the 23-tile macro ride is asserted, and the single-block ride's frame-to-frame displacement variation is bounded at 0.86px measured (2.5px bar). Failed designs, kept for the record in the class comment: raw contact aggregation (blinks), coyote-latched pushes (overshoots a young wave; luck decides), fold-owned Y (rider surfs down the wave's forward face), summed movers (one front, not additive), and the cliff-shaped proximity fade driving a soft X-servo + hard Y-clamp + fold hover (five controllers whose composition read as jitter). Scenario gotchas the macro test documents: pre-solid terrain adjacent to a pending sprout set promotes it immediately (a second wavefront grows backward and crushes the rider between fronts), and a growing shelf conveys a rider off its far edge however long it is. Remaining: guided maneuvers (`PredictGuided`, absolute gate + static spring frame), `TrajectoryLm`'s absolute gate, walk-drive tangential carry under input (the moving-platform half of 5.7), and time-dependent envelope y(x,t) if planning over moving terrain is ever needed. |
...
| 5.18 | Streamed chunks are never evicted. On the endless `spires` stage `ChunkMap.EnsureChunk` only ever adds — a player travelling in one direction accretes ~9 chunks/s (~35 KB/s), and nothing ever drops one. Fine for a session; unbounded over a long one. | **OPEN** | Eviction is not a one-liner, which is why it isn't done: the terrain journal's `RevertTile` silently no-ops on a missing chunk, so evicting a chunk that a live snapshot's journal still references would turn a rollback into silent terrain loss. Any eviction pass has to be gated on the oldest un-trimmed journal mark (and must XOR the chunk out of `TerrainHash`, as `DropChunk` does). |
