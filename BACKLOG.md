...
| 3.12 | Charged blocks: the payload is unwritten | **OPEN** | The gesture shipped 2026-08-30 — hold RMB in terrain to fill the avalanche meter, release, double-click a block: the whole meter buys one binary flag on that cell (`World/TileCharge.cs`, `BlockEruptionHelpers.TryChargeBlock`, `BuildMeters.BlockChargeMin`), drawn as a white fill tint + white gutter (`ChunkRenderer`). Nothing in the sim reads the flag yet, so today a charged block is a full meter spent on a colour. Decide what it *does* (detonate? amplify a nearby eruption? survive a break? propagate along a chain?) before tuning the gate. `BlockChargeTests`. |
