...
| 5.17 | The metaball/density stack (`SkeletonMetaballRenderer`, `DensityField`, `PrimitiveBatch`+`Primitives`, `Content/CapsuleSplat.fx`, `Content/MetaballComposite.fx`) is built every boot but reaches the screen only via `DebugDraw*Demo` flags that default off — no production consumer. Also: `GlowCore.Sphere` is never passed, so `GlowRenderer.cs`'s `CoreSphere` branch is unreachable. | **OPEN** | Decide: wire it as a character renderer or move it to `MTile.FxLab` and stop constructing it in `Game1.LoadContent`. Note `configs/game_config.json` ships with `DrawPlayerSprites`/`DrawPlayerSpriteSkin`/`DebugDrawSkeleton` all off and `DebugDrawBodies` on — the default visual is the debug polygon. |
