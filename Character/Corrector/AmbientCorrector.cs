...
public static class AmbientCorrector
{
                      ...
                      in FoldProfile fold, ref MovementVars vars)
    {
        ...
        // Lattice engine ("lattice", FoldLattice): the ref engine's regime and
        // tail, with the carry reference generated by the lattice path DP
        // instead of the hand-written rollout (Plans/LATTICE_PATH_PLANNER.md).
        if (fold.Fold && cfg.FoldEngine == "lattice"
            && FoldLattice.TryApply(ctx, s, fold, policy, dir, ref vars))
            return;
