...
public struct FoldProfile
{
    ...
    // the body's height (obstacle avoidance only) and the engine has no
...
public static class AmbientCorrector
{
                      ...
                      in FoldProfile fold, ref MovementVars vars)
    {
        ...
        // Lattice engine ("lattice", Plans/LATTICE_PATH_PLANNER.md): the
        // lattice path DP gives the trajectory's shape; LatticeTracker picks
        // this tick's channel forces from its first step. Nothing below runs.
        if (fold.Fold && cfg.FoldEngine == "lattice")
        {
            LatticeTracker.Apply(ctx, s, fold, dir, ref vars);
            return;
        }
