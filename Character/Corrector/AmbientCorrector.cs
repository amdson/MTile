...
public static class AmbientCorrector
{
                      ...
                      in FoldProfile fold, ref MovementVars vars)
    {
        ...
        if (fold.Fold && !launched)
        {
            ...
            // Lattice engine (FoldEngine "lattice", Plans/LATTICE_PATH_PLANNER.md
            // §1 revised note): the SAME qp flow — coast, obstacle rows,
            // progress rows, channel stack, solve — with the hover reference
            // taken from the lattice path DP instead of the floor envelope's
            // climb-band / down-anchor rules. dir == 0 keeps the envelope
            // hover column (§4.6); a solve with no path (seed pinned inside
            // an obstacle's margin) keeps it too.
            bool latticeRef = false;
            if (cfg.FoldEngine == "lattice" && dir != 0 && anchored)
            {
                int count = s.Lattice.Solve(ctx.Chunks, ctx.Body.Polygon,
                    ctx.Body.Position, ctx.Body.Velocity, new Vector2(dir, 0f),
                    hover: true, fold.HoverOffset, s.LatticePath, out _, out _);
                if (count > 0)
                {
                    latticeRef = true;
                    EmitLatticeReference(ctx, s, fold, n, lim, target, x0, dir,
                                         trackProgress, count, ref rowCount);
                }
            }
            if (!latticeRef)
        ...
        if (fold.Fold)
        {
            ...
            // Legs-at-support mask (plan §1 revised note): under the lattice
            // engine the LegServo would mean "body at or below the hover line"
            // (+ one lattice cell of quantization) instead of "a floor within
            // leg reach". Plumbed, OFF — measured 2026-08-24 with the lattice
            // reference + this stack (LATTICE_SCENARIOS.md "channel stack
            // results"): it keeps the free-standing 2-high wall honest (row 7)
            // but halves bumpy-corridor speed (65.8 → 36.6 px/s) and lets the
            // rest bob (vy 6.1). A design choice for the user, not a knob.
            const bool LegsAtSupport = false;
            float supportHover = LegsAtSupport && cfg.FoldEngine == "lattice"
                ? fold.HoverOffset + (float)Chunk.TileSize / Math.Clamp(cfg.LatticeCellsPerTile, 2, 8)
                : float.NaN;
            p.ChannelCount = CorrectorChannels.BuildFold(s, n, rowCount, dir,
                fold.MaxSpeed * ctx.Modifiers.MaxWalkSpeed, supportHover);
    ...
    // The lattice reference (FoldEngine "lattice"): EmitEnvelope's progress
    // rows unchanged; the hover row's target is the path's y at xRef —
    // interpolated along the polyline in p = dot(pos, u), which every DP edge
    // strictly increases, so y(p) is a function. Before the first node the
    // first node's y; past the last node (a bonk — the path ended at an
    // obstacle) the last node's y, i.e. the honest carry: progress rows keep
    // pulling, the coast's obstacle rows / truncation deliver the stop. No
    // climb band, no down-anchor rate limit, no elective classification —
    // the path's shape is the reference, the channel caps bound delivery.
    private static void EmitLatticeReference(
        EnvironmentContext ctx, CorrectorScratch s, in FoldProfile fold,
        int n, int lim, float target, float x0, int dir, bool trackProgress,
        int count, ref int rowCount)
    {
        var cfg = MovementConfig.Current;
        ...
        for (int k = 0; k < lim && rowCount < ClearanceConstraintBuilder.MaxEvents - 1; k++)
        {
            ...
            float pRef = xRef * dir;
            while (seg < count - 1 && path[seg].Pos.X * dir < pRef) seg++;
            float yRef;
            if (count == 1 || pRef <= path[0].Pos.X * dir) yRef = path[0].Pos.Y;
            else
            {
                float pa = path[seg - 1].Pos.X * dir, pb = path[seg].Pos.X * dir;
                if (pRef >= pb) yRef = path[seg].Pos.Y;
                else
                {
                    float t = (pRef - pa) / MathF.Max(pb - pa, 1e-6f);
                    yRef = path[seg - 1].Pos.Y + (path[seg].Pos.Y - path[seg - 1].Pos.Y) * t;
                }
            }
            s.RefY[k] = yRef;
