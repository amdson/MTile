...
| `BLOCK_THROW_PLAN.md` | **Built through Phase 5** (2026-08-28, branch `worktree-block-throw-plan`). `todo.txt` #2 — the block-grab *throw* half: the pull is routed through a `PullPointEntity` the action drives and hands off on release (so a follow-up action can't kill an undecided peel), the clod is a tracking `LobbedAreaProjectile` that detaches at the swipe velocity, textured orb sprite, trail + landing splash, dissipation knob. §4 is the critique of the original sketch; the header lists as-built deviations. Open: playtest tuning (`BACKLOG.md` 3.11). |
