<#
.SYNOPSIS
  Wire the built sound clips in Assets/Sounds into the game. (Windows; the
  macOS/Linux equivalent is sync-sounds.sh — keep the two in step. Both must
  emit BYTE-IDENTICAL output, or alternating between machines churns the diff.)
...
$lines.Add($BeginMark)
$lines.Add('# Written by scripts/sync-sounds (.ps1/.sh) — do not hand-edit; your changes will be lost.')
$lines.Add('')
foreach ($c in $clips) {
