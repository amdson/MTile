...
- **Combat is direct HP damage** (changed 2026-08-30 from a Smash-style escalation percent). `MaxHealth` 5; a hit takes `Hitbox.BodyDamage` straight off, a stock slash being 0.5. `Hitbox.Damage` is the TILE number and `BodyDamage` defaults to it — they differ only for attacks that must carve stone to work (Zeus's beams, the rail bolt). Crush impact into terrain is a second HP channel, not the only one. Knockback is a flat property of the attack; `StabAction` is the designated launcher and every other move sits well below it.
- **World reactions go through events** (`ChunkMap.OnTileBroken`), not polling.
- **Sim-side code never draws.** `ActionState`, `EnemyState<TVars>`, and `ITelegraphSource` entities declare overlay shapes via `Telegraph(TelegraphList t, …)` (`Presentation/TelegraphList.cs` — no graphics types); `Drawing/TelegraphRenderer` draws the list. Don't hand a `SpriteBatch` into `Character/` or `Entities/`, and don't add a second primitive helper — `DrawContext` is the one immediate API.
