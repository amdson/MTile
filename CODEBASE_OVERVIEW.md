...
**The combat model is direct HP damage** (changed 2026-08-30; it was Smash-style escalation before, see below). `OnHit` runs, in order: tech i-frames → Guard parry (`CombatState.ResolveGuard`) → grab-struggle (`GrabStrengthDamage` erodes the grabber's hold instead of dealing anything) → otherwise `Health -= hit.BodyDamage * guard.DamageScale` and flat knockback via `HitResolver.Resolve`. `MaxHealth` is 5; a stock slash is 0.5, a creature's melee swing 1.0. `Combat.DamageTaken` is a monotonic tally of HP lost this life (nothing in the sim branches on it — it exists because `Health` regenerates and so cannot answer "was this player hurt").

`Hitbox.Damage` is what a hit carves out of a **tile**; `Hitbox.BodyDamage` is what it takes off a **body**, and it defaults to `Damage`. Only the attacks whose tile number is pinned from below by a material's MaxHP set them apart — Zeus's bolt/sweep/column and the rail bolt all have to carve at 2.1–3.6 to clear a stone cell, which is most of a player's health bar.

**Crush** is the second HP channel, not the only one any more: `PhysicsBody.LastImpulseMagnitude` above `CrushImpulseThreshold` (700f) converts the excess into HP loss + hitstun, so a hard fall or a slam into a wall still bills you. HP trickles back at 0.15/s once you have been clear of damage (any source) for 3s. Post-hit invulnerability was **removed** (it made combos impossible) — only spawn protection and tech i-frames remain.

*What escalation was, and why it went:* a direct hit chipped no HP at all — it raised a monotonic `DamagePercent`, the percent multiplied incoming knockback (2.5× at 100%), and HP only came off when the resulting launch slammed you into terrain. Two things went wrong in play: knockback everywhere ended up enormous, and damage became indirect enough that a clean hit read as nothing happening. Removing it took the knockback multiplier with it; the authored strike speeds then came down ~40% on top of that (`StunImpulseThreshold` and `HitstunSecondsPerImpulse` were rescaled to match, so the same moves stun and the disadvantage windows are unchanged), with `StabAction` deliberately left alone so it reads as the kit's one launcher.
...
**Per-activation FSM state is plain data.** The FSM state/action instances are flyweights (one per registry entry, shared across activations); all mutable per-activation fields live in the [`MovementVars`](Character/Movement/MovementVars.cs)/[`ActionVars`](Character/Action/ActionVars.cs) value structs on the player, passed `ref` into lifecycle methods (`Enter`/`Update`/`Exit`/`CheckConditions`) and `in` into the read-only hooks. This is the snapshot unit — a struct copy. The eruption rework removed the last reference-typed action buffer, so there is no longer any per-action deep-copy special case. `BlockGrabAction` keeps only an `EntityId` in `ActionVars`: its peel group lives on a [`PullPointEntity`](Entities/PullPointEntity.cs) as a sparse `[InlineArray(25)]` `PeelGroupComp` value component (only point entities carry one), marshalled in the entity's `CaptureState`/`RestoreState` overrides — the pattern for anything that must outlive a button release (see `Plans/BLOCK_THROW_PLAN.md` §4.3). `PlayerAbilityState` holds the rest: `Facing`, `HasDoubleJumped`, ledge-grab flags, the [`BuildMeters`](Character/Action/BuildMeters.cs), and the nested `Condition`/`Combat` states.
...
- **Peel + throw** (`BlockGrabAction`, Shift+LMB with the cursor in solid, 6-tile reach) — the block-peel tether. The action is thin: on the press it spawns a [`PullPointEntity`](Entities/PullPointEntity.cs) and *drives* it while LMB is held (cursor → `TargetPos`, body → `OwnerPos`), then *hands it off* on release at the cursor's swipe velocity (an EMA in `ActionVars`) and exits that frame. The point owns the mechanics: a Gaussian kernel around the target admits cells into a group; a spring from target to group centre-of-mass pulls with `force = coeff · (dist/TileSize)^power`; force is divided among tether shares, wearing down per-member glue derived from material weight × outward solid edges. Overpull snaps (attempt dies); enough force breaks the group out into a `LobbedAreaProjectile` in its **Tracking** phase — a body of its own (tiles on, gravity off, no hurtbox) following the point with a velocity-matching `SmoothPen` tracker, resting near the hand (orbit at `GrabHandDistance` + tanh lean) and bleeding its budget over `GrabDissipateSeconds`. After hand-off the point flies straight (no drag) and finishes any live contest; the ball converges to the point's velocity and detaches into the ordinary lob — so a still release drops the clod and a swipe throws it at swipe speed, whether the blocks were in hand or still in the ground when the button came up. Point↔ball are linked by `EntityId`, resolved through `IEntitySpawner.Resolve`. Gated by `MovementConfig.BlockPeelEnabled` (default true; off = legacy drag-rip, also routed through the point). Design + critique: `Plans/BLOCK_THROW_PLAN.md`.
...
| Tune the block peel / throw | `Peel*` and `Grab*` knobs in `configs/movement_config.json` (hot-reload); mechanics in [PullPointEntity.cs](Entities/PullPointEntity.cs) + the Tracking phase of [LobbedAreaProjectile.cs](Entities/Projectiles/LobbedAreaProjectile.cs); `MovementConfig.BlockPeelEnabled` toggles legacy drag-rip |
